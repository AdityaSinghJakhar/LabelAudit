...
import com.labelguard.app.pipeline.Evaluation
...
data class ScanReport(
    ...
    /**
     * Names the checks that could not be assessed at all.
     *
     * A FAIL verdict outranks an unassessable check, because a proven
     * violation stays proven. But the reader must not take that verdict for a
     * complete audit: something on this pack was not read, and it might have
     * been another violation.
     */
    val partialAssessmentCaveat: String
        get() {
            val blocked = fields.flatMap { it.checks }
                .filter { it.status == RuleStatus.NOT_ASSESSABLE }
            if (blocked.isEmpty()) return ""
            return " ${blocked.size} check(s) could not be assessed from these " +
                "photographs, so this is a partial audit: " +
                blocked.joinToString(", ") { it.ruleId } + "."
        }
    ...
    val verdictExplanation: String
        get() = when (verdict) {
            Verdict.PASS -> "Every check that applied to this pack passed." +
                notApplicableCaveat + partialAssessmentCaveat
            Verdict.FAIL ->
