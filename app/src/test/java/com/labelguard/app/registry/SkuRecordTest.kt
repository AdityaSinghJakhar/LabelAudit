...
class SkuRecordTest {
    ...
    fun `applying a record populates the ruleset registry`() {
        val ruleset = com.labelguard.app.pipeline.Ruleset.load(
    ...
    fun `an applied record turns registry checks into real comparisons`() {
        ...
        val matching = com.labelguard.app.pipeline.RulesEngine.evaluate(
            applied, mapOf("mrp" to field("140"))
        ).findings.first { it.ruleId == "MRP-02" }
        assertEquals(com.labelguard.app.pipeline.RuleStatus.PASS, matching.status)

        // An overprinted price is now compared rather than skipped. It is not
        // failed, though: this reference came off a scanned pack, so the
        // disagreement is real but unresolved.
        val overprinted = com.labelguard.app.pipeline.RulesEngine.evaluate(
            applied, mapOf("mrp" to field("999"))
        ).findings.first { it.ruleId == "MRP-02" }
        assertEquals(
            com.labelguard.app.pipeline.RuleStatus.NEEDS_REVIEW,
            overprinted.status
        )
    }

    @Test
    fun `a scanned reference never gains authority to fail a pack`() {
        val ruleset = com.labelguard.app.pipeline.Ruleset.load(
            java.io.File("src/main/assets/ruleset.yaml").inputStream()
        )

        assertEquals(
            com.labelguard.app.pipeline.Ruleset.Authority.ASSERTED,
            Enrolment.applyTo(ruleset, gokul).registry.authority
        )
    }

    @Test
    fun `an imported product master can fail a pack`() {
        val ruleset = com.labelguard.app.pipeline.Ruleset.load(
            java.io.File("src/main/assets/ruleset.yaml").inputStream()
        )
        val imported = Enrolment.applyTo(
            ruleset,
            gokul.copy(source = RegistrySource.IMPORTED)
        )

        assertEquals(
            com.labelguard.app.pipeline.Ruleset.Authority.AUTHORITATIVE,
            imported.registry.authority
        )
        assertEquals(
            com.labelguard.app.pipeline.RuleStatus.FAIL,
            com.labelguard.app.pipeline.RulesEngine.evaluate(
                imported, mapOf("mrp" to field("999"))
            ).findings.first { it.ruleId == "MRP-02" }.status
        )
    }
