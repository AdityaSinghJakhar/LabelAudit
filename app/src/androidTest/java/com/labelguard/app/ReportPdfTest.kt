...
class ReportPdfTest {
    ...
    private fun report(): ScanReport {
        ...
        val evaluation = RulesEngine.evaluate(
            ...
            RulesEngine.Context(
                ...
                // A context has to say which of the two real states it is in.
                // Leaving both height fields empty meant "this device can
                // measure but found no character box", which is not what this
                // fixture is about — and it dragged the whole mrp group down
                // to NOT_ASSESSABLE. Modelling a phone that cannot measure at
                // all keeps the fixture on the question it was written for.
                heightScaleUnavailable = "this fixture has no camera behind it"
            )
        )
    ...
    fun rulesetLoadsFromAssets() {
        ...
        // Named rather than counted. A bare number told us only that the
        // total had moved, never which rule appeared or vanished, and the
        // set is what a reader actually wants when this breaks.
        assertEquals(
            setOf(
                "MRP-01", "MRP-02", "TAX-01",
                "QTY-01", "QTY-02",
                "BRAND-01", "MFR-01", "CARE-01",
                "MFG-01", "MFG-02",
                "BATCH-01",
                "EXPIRY-01", "EXPIRY-02", "EXPIRY-03",
                "FSSAI-01",
                "CAP-01", "CAP-02"
            ),
            rules.rules.map { it.id }.toSet()
        )
    }
    ...
    @Test
    fun anUnmeasurableHeightDoesNotMakeAFieldLookUnassessable() {
        // CAP-01 bears on mrp. On a phone that cannot measure character
        // height at all, that is a fact about the equipment and must leave
        // the declaration itself reading as read-and-passed.
        val mrp = report().fields.first { it.field == "mrp" }
        val height = mrp.checks.first { it.ruleId == "CAP-01" }
